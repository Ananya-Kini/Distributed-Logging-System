...
async def register_patient(patient: PatientCreate, db: Session = Depends(get_db)):
    try:
        ...
        if not patient.name or len(patient.name.strip()) < 2:
            raise ValueError("Patient name must be at least 2 characters long")
        
        if not patient.condition or len(patient.condition.strip()) < 5:
            raise ValueError("Patient condition description must be at least 5 characters long")
